...
class ProcessLogs:
    ...
    def run(self, global_rdr):
        ...
        try:
            cnt = len(dict_logtext)
        except TypeError:
            print("No file in the directory {}".format(self.path_logs))
            return 1
        ...
        problems_logs_ids = []
        no_result_logs_ids = []
        healthy_training_set = []

        # Create folder for moving the already processed logfiles
        path_done = os.path.join(self.path_logs, 'done')
        path_ner_error = os.path.join(self.path_logs, 'ner_error')

        if not os.path.exists(path_done):
            os.makedirs(os.path.join(path_done))
        if not os.path.exists(path_ner_error):
            os.makedirs(os.path.join(path_ner_error))

        # print colour for DEBUG
        ...
        for logpath, log_info in dict_logtext.items():
            ...
            print("\n************************************************************************************************" \
                  "**************************************************************************")
            printLightPurple("\nProcessing log num. " + str(idx) + " of " + str(cnt) + " total logs.")
            idx += 1
            self.fname = re.sub(self.path_logs, '', logpath)
            self.fname = re.sub('.log', '', self.fname)
            ...
            try:
                ...
                printRed("\n\n ERROR: An exception of type {0} occurred. \nArguments: {1!r}"
                      .format(type(e).__name__, e.args))
                # If there's an error, just move error log to new folder
                src = logpath
                dst = os.path.join(path_ner_error, self.fname + '.log')
                shutil.move(src, dst)
                continue
            ...
            if result:
                ...
                if result_quality == "healthy":
                    ...
                    healthy_logs_ids.append(result['log_file_name'])

                    healthy_training_set.append(result)
                    # put visual log here
                elif result_quality == "problems":
                    self._save(result, self.fname, subfolder="problems")
                    problems_logs_ids.append(result['log_file_name'])
                    ...
                    corrupt_logs_ids.append(result['log_file_name'])
            else:
                self._save("no result!", self.fname, subfolder="no_result")
                no_result_logs_ids.append(self.fname)

            # Move already processed logs to new folder
            src = logpath
            dst = os.path.join(path_done, self.fname + '.log')
            shutil.move(src, dst)

        # Analysis of logs
        try:
            percent_healthy = (len(healthy_logs_ids) / cnt) * 100
        except ZeroDivisionError:
            print('No dialogue logs found in {}'.format(self.path_logs))
            return 1
        percent_corrupt = (len(corrupt_logs_ids) / cnt) * 100
        percent_problems = (len(problems_logs_ids) / cnt) * 100
        percent_no_result = (len(no_result_logs_ids) / cnt) * 100
        overview = ("healthy logs - {0:.1f}%, "\
                   "corrupt logs - {1:.1f}%, " \
                   "problem logs - {2:.1f}%, " \
                   "no_result logs - {3:.1f}%").format(percent_healthy, percent_corrupt,
                                                        percent_problems, percent_no_result)
        overview_of_logs = {
            'total_number_of_logs' : cnt,
            'healthy_logs_ids' : healthy_logs_ids,
            'number_of_healthy_logs' : len(healthy_logs_ids),
            'corrupt_logs_ids': corrupt_logs_ids,
            'number_of_corrupt_logs' : len(corrupt_logs_ids),
            'problems_logs_ids': problems_logs_ids,
            'number_of_problems_logs' : len(problems_logs_ids),
            'no_result_logs_ids': no_result_logs_ids,
            'number_of_no_result_logs': len(no_result_logs_ids),
            'overview': overview
            }
        self._save(overview_of_logs, "log overview", subfolder="log_overview")
        # Save all heathy samples
        self._save(healthy_training_set, "healthy_training_set", subfolder="healthy")
    ...
    def _processed_dialogue(self, _users, _log_text):
        ...
        # -------------------------------------------------------------------------------------------------------------
        for count, item in enumerate(_log_text):
        ...
        # -------------------------------------------------------------------------------------------------------------
        self.original_dialogue_ner_small, self.original_dialogue_ner_full, named_entity_dict, \
        ...
        """ FOR DEBUG """
        self.entities_dict_full = self.ner.entites_dict_full
        ...
        single_sample_output.update({
            ...
            'dialogue_ner': self.original_dialogue_ner_small,
            'dialogue_ner_full': self.original_dialogue_ner_full,
    ...
    def _save(self, result, filename, subfolder=""):
        ...
        # Save as json
        with open(os.path.join(self.path_results, subfolder, filename + '.txt'), 'w') as outfile:

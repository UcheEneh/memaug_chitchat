...
"""
    Combination of works from:
        - http://norvig.com/spell-correct.html
        - https://www.datasciencecentral.com/profiles/blogs/how-we-used-spacy-and-hunspell-to-handle-typos-in-an-ai-chatbot 
"""
...
class WordFrequency(object):
    ...
    def _update_dictionary(self):
        ...
        self._letters = set()   # set() creates a list of characters during update()
...
class SpellChecker(object):
    ...
    def correction(self, word):
        """ The most probable correct spelling for the word
            ...
            correction = argmax(c in suggestions) P(c|w)
            
            By Bayes' Theorem this is equivalent to:
            argmax(c in suggestions) P(c) P(w|c) / P(w)
            
            Since P(w) is the same for every possible suggestion c, we can factor it out, giving:
            argmax(c in suggestions) P(c) P(w|c)

            The four parts of this expression are:
            - Selection Mechanism: argmax 
                We choose the suggestion with the highest combined probability.
                
            - Suggestion Model: c in suggestions
                This tells us which suggestion corrections, c, to consider.
            ...
            Returns:
                str: The most likely suggestion
        """
        all = self.suggestions(word) 
        
        # return two most probable corrections if available
        if len(all) > 1:
            sugg1 = max(all, key=self.word_probability)
            all.remove(sugg1)
            sugg2 = max(all, key=self.word_probability)
            return [sugg1, sugg2]
        # else reurn just the one
        else:
            return [max(self.suggestions(word), key=self.word_probability)]
    ...
    def suggestions(self, word):
        """ Generate possible spelling corrections for the provided word up to
            ...
            Args:
                word (str): The word for which to calculate suggestion spellings
            Returns:
                set: The set of words that are possible suggestions 
        """
    ...
    # Returns word if is in the big dictionary
    ...
    # Returns word if not in big dictionary   
